...
df['Close'] = np.random.normal(loc=0.0002, scale=0.011, size=PERIODS)
...
df['100MAdist'] = (df['Close'] - df['100MA'])/df['100MA']
...
true_dates = df[df['MAsignal'] == 1].index.tolist()
...
df_ohlc = pd.DataFrame(index=our_index_half_hour)
...
df_ohlc['Close'] = np.exp(df_ohlc['Close'])
df_ohlc['Close'].iloc[0] = 1
df_ohlc['Close'] = df_ohlc['Close'].cumprod()
...
df_ohlc = df_ohlc['Close'].resample('1D').ohlc()
...
df_ohlc['100MA'] = df_ohlc['Close'].rolling(100).mean()
df_ohlc['MAsignal'] = np.where(df_ohlc['Close'] > df_ohlc['100MA'], 1, 0)
...
condition2 = (df_ohlc['MAsignal'].shift(1) == 1) & (df_ohlc['MAsignal'].shift(2) == 1)
condition3 = (df_ohlc['MAsignal'].shift(1) == 0) & (df_ohlc['MAsignal'].shift(2) == 1)
...
df_ohlc['PortChng'] = 0
df_ohlc['PortChng'] = np.where(condition1, -1 + df_ohlc['Close']/df_ohlc['Open'], df_ohlc['PortChng'])
df_ohlc['PortChng'] = np.where(condition2, -1 + df_ohlc['Close']/df_ohlc['Close'].shift(1), df_ohlc['PortChng'])
df_ohlc['PortChng'] = np.where(condition3, -1 + df_ohlc['Open']/df_ohlc['Close'].shift(1), df_ohlc['PortChng'])

# In order to get the accumulated change in portfolio we will have to add 1 to 'PortChng' and use cumprod.
df_ohlc['Port'] = df_ohlc['PortChng'] + 1
df_ohlc['Port'] = df_ohlc['Port'].cumprod()
