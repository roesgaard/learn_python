...
print(f"Exercise 1 - {x.strftime('%x')}")
print(f"Exercise 1 - {x.strftime('%X')}")
...
print(f"Exercise 2 - {x.strftime('%Y-%m-%d %H:%M')}")
...
g = datetime.datetime(1400, 7, 15, 15, 21)
print(f'Exercise 3 - {g}')
...
to_dt = datetime.datetime(2021, 1, 1)
...
while from_dt < to_dt:
    ...
    from_dt = end
    weeks.append((i, beg, end))
    i += 1

# print just a few weeks
for w in weeks[5:10]:
    print(f'week {w[0]:2} - {w[1].strftime("%x")} - {w[2].strftime("%x")}')
...
for w in weeks[5:]:
    x = f'week {w[0]:2} - {w[1].strftime("%x")} - {w[2].strftime("%x")}'
    if w[1] <= today <= w[2]:
        x += ' * current week'
    print(x)
...
for x in range(1, 13):
    d = datetime.date(2020 + int(x/12), x % 12 + 1, 1)  # 1st of month x+1
    d = d - datetime.timedelta(days=1)  # last day of month x
    while d.weekday() > 4:
